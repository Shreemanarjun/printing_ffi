...
class NetworkPrintingCard extends StatelessWidget {
  const NetworkPrintingCard({
    super.key,
  ...
  Widget build(BuildContext context) {
    final theme = ShadTheme.of(context);
    return ShadCard(
      title: Text('Direct Network Printing', style: theme.textTheme.h4),
      description: const Text(
        'Send raw data directly to a printer using its IP address and port. '
        'This bypasses the OS print spooler and is ideal for receipt or label printers.',
      ),
      child: Padding(
        padding: const EdgeInsets.only(top: 24),
        child: Column(
          ...
          children: [
              ...
              children: [
                Expanded(
                  ...
                  child: ShadInput(
                    ...
                    placeholder: const Text('Printer IP Address'),
                  ),
                ),
                const SizedBox(width: 16),
                Expanded(
                  ...
                  child: ShadInput(
                    ...
                    placeholder: const Text('Port (e.g., 9100)'),
                  ),
                ),
              ],
            ),
            ...
            Row(
              ...
              crossAxisAlignment: CrossAxisAlignment.center,
              children: [
                Text('Data Type', style: theme.textTheme.large),
                const SizedBox(width: 16),
                SizedBox(
                  width: 150,
                  child: ShadSelect<RawDataType>(
                    ...
                    initialValue: selectedRawDataType,
                    onChanged: onRawDataTypeChanged,
                    options: RawDataType.values.map((e) => ShadOption(value: e, child: Text(e.label))).toList(),
                  ),
                ),
              ],
            ),
            const SizedBox(height: 12),
            ShadInput(controller: dataController, placeholder: const Text('Enter raw data... e.g., for ZPL or ESC/POS'), maxLines: 5),
            const SizedBox(height: 20),
            ShadButton(leading: const Icon(Icons.send_to_mobile, size: 16), onPressed: onPrint, child: const Text('Send Data to Network Printer')),
          ],
        ),
      ),
    );
